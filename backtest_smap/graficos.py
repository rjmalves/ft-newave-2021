...
def grafico_ghid_subsistema(casos: List[Caso],
                            dir_saida: str):
    # Cria o objeto de figura
    fig, axs = plt.subplots(2, 2, figsize=(16, 9))
    fig.suptitle("Evolução da Geração Hidraulica por Submercado",
                 fontsize=14)
    for ax in axs.flat:
        ax.set(xlabel='', ylabel='Ghid (MWmed)')
    # Variáveis para limitar os eixos no futuro
    max_y = {s: 0 for s in SUBSISTEMAS}
    min_y = {s: 1e6 for s in SUBSISTEMAS}
    max_x = 0
...
def grafico_ghid_sin(casos: List[Caso],
                     dir_saida: str):
    # Cria o objeto de figura
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.set_title("Evolução da Geração Hidraulica para o SIN",
                 fontsize=14)
